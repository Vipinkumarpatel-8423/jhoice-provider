...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<List<EService>> searchEServices(
    ...
    var _queryParameters = {
      ...
      'search':
          'categories.type:provider,both;categories.id:${categories.join(',')};name:$keywords',
  ...
  Future<EService> getEService(String id) async {
    var _queryParameters = {
      ...
      'search': 'categories.type:provider,both;',
  ...
  Future<List<EService>> getEProviderFeaturedEServices(int page) async {
    var _queryParameters = {
      'with': 'eProvider;eProvider.addresses;categories',
      'search': 'categories.type:provider,both;featured:1',
      'searchFields': 'categories.type:in;featured:=',
      'searchJoin': 'and',
      'limit': '4',
  ...
  Future<bool> sendNotification(List<User> users, User from, String type,
      String text, String msg_id, String uId) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ sendNotification() ]");
  ...
  Future<bool> sendNotification2(List<User> users, User from, String type,
      String text, String msg_id, String uId) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
    ...
    print(data);
    Uri _uri =
        getApiBaseUri("new_notify").replace(queryParameters: _queryParameters);
    Get.log(_uri.toString());
    Get.log(data.toString());
    var response =
        await _httpClient.postUri(_uri, data: data, options: _optionsNetwork);
  ...
  Future<bool> addAvailibility(Map<String, dynamic> data) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ AddAvailibility ]");
  ...
  Future<List<AvailabilityHour>> getAvailabilityByProviderId(String id) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ AddAvailibility ]");
    ...
    Uri _uri = getApiBaseUri("availibility_eid")
        .replace(queryParameters: _queryParameters);
    ;
    printUri(StackTrace.current, _uri);
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
  ...
  Future<bool> deleteAvailabilityByProviderId(String id) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ AddAvailibility ]");
    ...
    Uri _uri = getApiBaseUri("availibility_delete")
        .replace(queryParameters: _queryParameters);
    ;
    printUri(StackTrace.current, _uri);
